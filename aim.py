...
dt = 1 / (4.0 * D + np.exp(beta)) #time discretization
prob_move = 2.0 * dt
...
for current_step in range(total_timesteps):
    ...
    for n in range(N):
        ...
        next_ptcl.append(Ptcl(next_pos, next_spin))  # Correctly creating Ptcl instances
...
ax.set_title('2D Active Ising Model (Density), Time = 0')
fig.colorbar(c)

 # Update the image data and title for each frame
def animate(i):
    c.set_data(coordinated_density[i].T)
    ax.set_title(f'2D Active Ising Model (Density), Time = {i*dt:.2f}')
    return c,
...
# Save the animation
ani.save(f'AIM_Density_{tf}.gif', writer='imagemagick')
...
# Initial frame setup
c = ax.imshow(coordinated_magnetization[0].T, animated=True, cmap='viridis', 
              extent=[0, Lx, 0, Ly], vmin=coordinated_magnetization.min(), vmax=coordinated_magnetization.max())
ax.set_title('2D Active Ising Model (Magnetization), Time = 0')
fig.colorbar(c)
...
def animate(i):
    ...
    ax.set_title(f'2D Active Ising Model (Magnetization), Time = {i*dt:.2f}')
    return c,
...
# Save the animation
ani.save(f'AIM_Magnetization_{tf}.gif', writer='imagemagick')
