...
ptcl = [Ptcl(random.randint(0, L2 - 1), 1) for _ in range(N)]
...
ax.set_title('Liquid Spinodal Decomposition (Density), Time = 0')
fig.colorbar(c)
...
def animate(i):
    ...
    ax.set_title(f'Liquid Spinodal Decomposition (Density), Time = {i*dt:.2f}')
    return c,
...
# Save the animation
ani.save(f'liq_spin_den_{tf}.gif', writer='imagemagick')
...
ax.set_title('Liquid Spinodal Decomposition (Magnetization), Time = 0')
fig.colorbar(c)
...
def animate(i):
    ...
    ax.set_title(f'Liquid Spinodal Decomposition (Magnetization), Time = {i*dt:.2f}')
    return c,
...
# Save the animation
ani.save(f'liq_spin_mag_{tf}.gif', writer='imagemagick')
